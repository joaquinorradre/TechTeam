...
class GetTopOfTheTops extends Controller
{
    public function fetchData(Request $request)
    {
        ...
        {
            $accessTokenUrl = "https://id.twitch.tv/oauth2/token?client_id={$clientId}
        ...
        function fetchAndInsertVideos($gamesResponse, $clientId, $accessToken)
        {
            foreach ($gamesResponse['data'] as $game) {
                ...
                echo $videosResponse ? insertVideos($videosResponse, $game['id']) : "No se encontraron videos 
		para el juego con ID: {$game['id']}<br>";
            }
        }


        function insertVideos($videosResponse, $idGame)
        {
        ...
        function obtenerTiempoDesdeUltimaActualizacion($idGame)
        {
        ...
        if ($gameCount == 0 && isset($accessToken)) { // No hay datos en la base de datos
            ...
            if (isset($gamesResponse['data'])) {
                ...
                foreach ($games as $game) {
                    $idGame = $game['id'];
                    $name = $game['name'];
                    $existingGame = DB::table('Game')->where('game_id', $idGame)->first();
                    if (!$existingGame) {
                            DB::table('Game')->insert([
                            'game_id' => $game['id'],
                            'game_name' => $game['name'],
                        ...
                        $videosUrl = "https://api.twitch.tv/helix/videos?game_id={$game['id']}&sort=views&first=40";
                        $videosResponse = fetchTwitchData($videosUrl, $clientId, $accessToken);
                        insertVideos($videosResponse, $idGame);
                        $results = DB::table('Video as v')
                        ->select(
                            'v.game_id',
                            'g.game_name',
                        ...
                        if ($request->has('since')) {
                            ...
                            $resultado = obtenerTiempoDesdeUltimaActualizacion($idGame);
                            if ($resultado < $since) { //NO HA PASADO EL TIEMPO MAXIMO
                                array_push($array, menosTiempoMax($idGame)); //Consulta
                            } else { //HA PASADO EL TIEMPO MAXIMO
                                array_push($array, masTiempoMax($idGame, $accessToken)); //Borrar y volver a poner
                            }
                        } else {
                            $since = 10 * 60;
                            $resultado = obtenerTiempoDesdeUltimaActualizacion($idGame);
